...
the graphic itself. For this we use the \icode{rex} shader in \icode{camel.gas}.
This shader is capable of shearing as well as scaling the image. For our purposes
here all we need to do is pass the scaling parameters we calculated above to the
\icode{shearx} and \icode{sheary} fields in the GPU RAM input buffer \icode{in\_buf}.
